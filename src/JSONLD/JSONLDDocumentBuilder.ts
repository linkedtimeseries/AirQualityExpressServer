...
export default class JSONLDDocumentBuilder {
    ...
    constructor(tile: ITile, observationTimeQuery: string) {
        this.zoom = tile.zoom;
    ...
    private buildTilesInfo(): string {
        let ti: string = "";
        ti += '"@id":"' + JSONLDConfig.openobeliskAddress +
            this.zoom.toString() +
            "/" +
            this.longitudeTile.toString() +
            "/" +
            this.latitudeTile.toString() +
            "/" +
            this.observationTimeQuery +
            '"';
